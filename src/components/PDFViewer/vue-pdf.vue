  ...
  setup(props, ctx) {
    ...
    const initPdfWorker = () => {
      ...
      loadingTask.promise.then((pdf) => {
        ...
        if (pageNumber.value <= numberOfPages.value) {
          pdf.getPage(pageNumber.value).then((page) => renderPage(page));
        }
      });
    };
    ...
    const callback = function (entries, observer) {
      ...
      if (entry.isIntersecting) {
        ctx.emit("onScrollPageChanged", entry.target.id);
      }
    };
    ...
    const renderPage = async (page) => {
      ...
      // Observe page
      observer.observe(parentWrapperEl);
...
<template>
  <div :id="page" class="vue-pdf-main" ref="parentWrapperRef">
